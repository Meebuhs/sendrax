...
class ViewOnlyClimbScreen extends StatefulWidget {
  ViewOnlyClimbScreen(
      {Key key, @required this.climbId, @required this.climbName})
      : super(key: key);
...
class ViewOnlyClimbWidget extends StatelessWidget {
  ...
  List<Widget> _buildActions(BuildContext context) {
    return <Widget>[
      IconButton(
          ...
          onPressed: () => _showUnarchiveClimbDialog(context)),
      IconButton(
          icon: Icon(Icons.delete_forever),
          onPressed: () => _showDeleteClimbDialog(context))
    ];
  }
  ...
  Widget _showClimbInformation(ViewOnlyClimbState state, BuildContext context) {
    String gradeAndSectionText = (state.climb.section == null)
        ? "${state.climb.grade}"
        : "${state.climb.grade} - ${state.climb.section}";

    String categoriesText = (state.climb.categories.isNotEmpty)
        ? " - ${state.climb.categories.join(', ')}"
        : "";

    String attemptsText = " A: ${state.climb.attempts.length}";
              ...
              child: Padding(
                  ...
                  child: Row(
                    children: <Widget>[
                      ...
                      _createStatusIcon(state, context),
                      Text("$attemptsText",
                          style: Theme.of(context).accentTextTheme.subtitle2)
                    ],
                  ))),
  ...
  Widget _createStatusIcon(ViewOnlyClimbState state, BuildContext context) {
    IconData statusIcon;

    if (state.climb.repeated) {
      statusIcon = Icons.rotate_right;
    } else if (state.climb.sent) {
  ...
  Widget _buildDateCard(BuildContext context, ViewOnlyClimbState state,
      List<DateTime> dates, int index) {
    List<Attempt> attemptsOnDate = List.from(state.climb.attempts.where(
        (attempt) =>
    ...
    attemptItems.add(
      Padding(
        padding: EdgeInsets.fromLTRB(
            UIConstants.SMALLER_PADDING, UIConstants.SMALLER_PADDING, 0, 0),
        child: SizedBox(
              ...
              actions: <Widget>[
                ...
                TextButton(
                  child: Text("DELETE",
                      style: Theme.of(context).accentTextTheme.button),
                  onPressed: () =>
                      BlocProvider.of<ViewOnlyClimbBloc>(upperContext)
                          .deleteClimb(upperContext),
                )
              ]);
        });
  }
  ...
  void navigateToImageView(ImageProvider image) {
    NavigationHelper.navigateToImageView(
        widgetState.context, image, "viewOnlyClimbImageHero",
        addToBackStack: true);
  }
}
